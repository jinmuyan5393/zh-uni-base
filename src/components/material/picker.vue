    ...
    >
      <template v-if="!hiddenUpload" #trigger>
        <div class="material-select__trigger clearfix" @click.stop>
          <template v-for="(element, index) in fileList" :key="index">
            <div
          ...
          >
            <slot name="upload">
              ...
              >
                <baseIcon :size="25" name="el-icon-Plus" />
                <span>添加</span>
              </div>
            </slot>
          </div>
...
.material-select {
  ...
  .material-upload {
    :deep(.upload-btn) {
      @apply box-border rounded border-dashed border flex flex-col justify-center items-center;
    }
  }
}
.material-wrap {
