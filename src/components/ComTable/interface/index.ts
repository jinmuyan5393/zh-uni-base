...
export interface ComTableProps {
  ...
  tableData: any[]; // 表格数据
  isPageable?: boolean; // 是否展示分页
  pageInfo?: PageInfo; // 分页配置
  rowKey?: string; // 行数据的key
  border?: boolean; // 是否带纵向边框
  loading?: boolean; // 是否展示加载
  elementLoadingText?: string; // 加载时显示的文字
...
export type ComTableInstance = Omit<
  InstanceType<typeof ComTable>,
  keyof ComponentPublicInstance | keyof ComTableProps
>;
