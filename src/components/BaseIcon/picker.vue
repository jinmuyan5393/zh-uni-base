    ...
    <el-popover trigger="contextmenu" v-model:visible="state.popoverVisible" :width="state.popoverWidth">
      ...
      <template #reference>
        <el-input
          ...
          <template #prepend>
            <div class="flex items-center" v-if="modelValue">
              <el-tooltip class="flex-1 w-20" :content="modelValue" placement="top">
                <BaseIcon class="mr-1" :key="modelValue" :name="modelValue" :size="16" />
              </el-tooltip>
            </div>
          ...
          <template #append>
            <el-button>
              <BaseIcon name="el-icon-Close" :size="18" @click="handleClear" />
            </el-button>
          </template>
        </el-input>
      </template>
...
import { ref, computed, nextTick, onMounted, reactive, shallowRef, watch } from "vue";
