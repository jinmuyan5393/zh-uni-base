...
import { Setting, Check, Switch } from "@element-plus/icons-vue";

const menuRef = ref();
const defaultActive = ref(null);

const { t, route, locale, translationCh, translationEn } = useTranslationLang(menuRef);
const { device, logout, onPanel, resolvePath, username, avatar, getDivStyle, avatarsStyle, getDropdownItemStyle, getDropdownItemClass } = useNav();
  ...
  <div v-if="device !== 'mobile'" class="horizontal-header" v-loading="usePermissionStoreHook().wholeMenus.length === 0">
    <el-menu router ref="menuRef" mode="horizontal" class="horizontal-header-menu" :default-active="defaultActive">
      <el-menu-item v-for="route in usePermissionStoreHook().wholeMenus" :key="route.path" :index="resolvePath(route) || route.redirect">
        <template #title>
          <div v-if="toRaw(route.icon)" :class="['sub-menu-icon', route.icon]">
            <base-icon class="menu-item-icon" :size="16" :name="toRaw(route.icon)" />
          </div>
          <div :style="getDivStyle">
            <span class="select-none">
              {{ transformI18n(route.title) }}
            </span>
            <extraIcon :extraIcon="route.extraIcon" />
          </div>
        </template>
      </el-menu-item>
    </el-menu>
