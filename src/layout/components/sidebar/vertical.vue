...
const menuData = computed(() => {
  return appStore.layout === "mix" && device.value !== "mobile" ? subMenuData.value : usePermissionStoreHook().wholeMenus;
});

const loading = computed(() => (appStore.layout === "mix" ? false : menuData.value.length === 0 ? true : false));
...
<template>
  <div v-loading="loading" :class="['sidebar-container', showLogo ? 'has-logo' : '']">
    ...
    <leftCollapse v-if="device !== 'mobile'" :is-active="appStore.sidebar.opened" @toggleClick="toggleSideBar" />
  </div>
</template>
