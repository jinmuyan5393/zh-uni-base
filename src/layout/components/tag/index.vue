    ...
    case 6:
      ...
      setTimeout(() => {
        if (isFullscreen.value) {
          tagsViews[6].icon = "local-icon-fullscreen-exit";
    ...
    case 7:
      ...
      setTimeout(() => {
        if (pureSetting.hiddenSideBar) {
          tagsViews[7].icon = "local-icon-fullscreen-exit";
...
watch(isFullscreen, () => {
  tagsViews[6].icon = "local-icon-fullscreen";
  tagsViews[6].text = $t("buttons.hswholeFullScreen");
});
...
<template>
  <div ref="containerDom" class="tags-view" v-if="!showTags">
    <span v-show="isShowArrow" class="arrow-left">
      <BaseIcon name="el-icon-ArrowLeft" @click="handleScroll(-200)" />
    </span>
    ...
    <span v-show="isShowArrow" class="arrow-right">
      <BaseIcon name="el-icon-ArrowRight" @click="handleScroll(-200)" />
    </span>
    <!-- 右键菜单按钮 -->
